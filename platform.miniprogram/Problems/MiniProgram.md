...
​	web-view中，以链接形式跳转展示的 pdf 等文件（安卓会下载文件），如 "https://www.xxx.com/folder/index.pdf" 不能正常展示。
...
​	蒙层（position: fixed）滑动时，底部被蒙层遮住的内容也会滑动。
...
### 5. 分享返回时，触发 app.onShow 参数带有旧数据的问题
...
1. onShow 有参数且参数不同的两个动作先后触发时表现正常，不会出现该问题，如扫码进入小程序后跳转其他小程序；

2. 有参数变化的动作不会出现该问题。如先后跳转两个小程序返回，scene 相同但其他参数值不同；或分享返回小程序后跳转其他小程序后返回，分享返回无参数而小程序返回有参数。

**解决方案：**

​	新增全局变量用于判断是否已经完成。绑定后赋值为 true 。onShow 触发时先根据该变量判断是否已完成，已完成的不再进行相应处理。
